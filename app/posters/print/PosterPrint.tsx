...
// 1 design-px = 0.1cm in print  →  poster is exactly 50cm × 70cm
// Section heights (px → cm): hero 256→25.6, sep 4→0.4, books 250→25, cta 112→11.2, footer 78→7.8
...
export default function PosterPrint({ data }: { data: PosterData }) {
  ...
  // Wait for all images before enabling print button
  useEffect(() => {
    ...
    <>
      ...
      <div
        ...
        style={{
          ...
          fontFamily: font, borderBottom: '1px solid #333',
        }}
      >
        <span style={{ color: '#888', fontSize: 13, letterSpacing: 2, textTransform: 'uppercase' }}>
          {ready ? 'Ready to print' : 'Loading images…'}
        </span>
        <button
          onClick={() => window.print()}
          disabled={!ready}
          style={{
            background: ready ? '#C9923A' : '#444',
            color: '#fff', border: 'none', borderRadius: 4,
            padding: '10px 28px', fontSize: 14, fontWeight: 700,
            letterSpacing: 2, textTransform: 'uppercase', cursor: ready ? 'pointer' : 'not-allowed',
      ...
      {/* Spacer for fixed button bar (hidden in print) */}
